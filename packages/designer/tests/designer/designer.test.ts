...
describe('Designer 测试', () => {
  ...
  it('addPropsReducer / transformProps', () => {
    ...
    // props 是数组
    expect(designer.transformProps([{ num: 1 }], mockNode, TransformStage.Init)).toEqual([
      { num: 1 },
    ]);
    ...
    expect(designer.transformProps({ num: 1 }, mockNode, TransformStage.Serilize)).toEqual({
      num: 2,
    });
    expect(designer.transformProps({ num: 1 }, mockNode, TransformStage.Render)).toEqual({
      num: 2,
    });
    expect(designer.transformProps({ num: 1 }, mockNode, TransformStage.Save)).toEqual({ num: 2 });
    expect(designer.transformProps({ num: 1 }, mockNode, TransformStage.Upgrade)).toEqual({
      num: 2,
    });
